...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int n) {
        if (this.isLegal(player)) {
            return this.get(n).getSide().equals(WHITE)
                || this.get(n).getSide().equals(player);
        } else {
            return false;
        }
    }
    ...
    boolean isLegal(Side player) {
        return (player.equals(this.whoseMove()));
    }
    ...
    final Side getWinner() {
        int redCount = 0;
        int blueCount = 0;
        redCount = numOfSide(Side.RED);
        blueCount = numOfSide(Side.BLUE);
        if (redCount == this.size() * this.size()) {
            return Side.RED;
        } else if (blueCount == this.size() * this.size()) {
            return Side.BLUE;
        } else {
            return null;
        }
    ...
    public String toString() {
        ...
        for (int r = 1; r <= this.size(); r++) {
            ...
            for (int c = 1; c <= this.size(); c++) {
                result += this.get(r, c).getSpots();
                if (this.get(r, c).getSide().equals(RED)) {
                    result += "r";
                } else if (this.get(r, c).getSide().equals(BLUE)) {
                    result += "b";
                } else if (this.get(r, c).getSide().equals(WHITE)) {
                    result += "-";
                }
                result += " ";
            }
